...
import 'package:grocery_store/models/cobone_model.dart';
...
class _CartScreenState extends State<CartScreen> {
  ...
  Container coboneRow(Size size, BuildContext context) {
    return Container(
      height: 80.0,
      width: size.width,
          ...
          Expanded(
            ...
            child: Container(
              ...
              child: FlatButton(
                onPressed: () {
                  focusNode.unfocus();
                  for(var code in coboneList)
                  {
                    if(code.coboneCode==controller.text)
                    {

                      print("ok ${code.coboneCode}");
  ...
  getCobone() async {
    QuerySnapshot querySnapshot = await db.collection(Paths.cobone).get();
    var data = querySnapshot.docs;
    for(var e in data ){
      var data = e.data();
      coboneList.add(
        Cobone(
            cobonePercent: data[Paths.coboneprecent],
            coboneCode: data[Paths.coboneCode],
            coboneid: e.id),
      );
    }

    return print(coboneList[0].coboneCode);
